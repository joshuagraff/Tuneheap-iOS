...
-(void)parseData{
    if(stations.count==0){
        ...
        /**
        sleep(2);
        //NSMutableArray *stationLikes = stationPage.likes;
        for(int i =0;i<stationScrapers.count;i++){
            stationScreenScraper *curScraper = [stationScrapers objectAtIndex:i];
            for(int j = 0;j<curScraper.likes.count;j++){
                NSLog([curScraper.likes objectAtIndex:j]);
            }
            //[likes addObject:[stationPage.likes objectAtIndex:i]];
        }
        
        NSLog(@"Likes: ");
        for(int i =0;i<likes.count;i++){
            NSLog([likes objectAtIndex:i]);
        }*/
    }
